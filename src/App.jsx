...
import { Trophy, Settings, ChevronRight, Lock, History, Send, Layers, Loader2, Sparkles, Wand2, ArrowLeft, Zap, XCircle, CheckCircle, Upload } from "lucide-react";
            ...
            {!settingsOpen ? (
              ...
              <div style={{display:"flex", flexDirection:"column", gap:16}}>
                ...
                <div style={{background:"#fff", border:"1px solid #E8E5DC", borderRadius:16, padding:16}}>
                  <p style={{fontSize:10, color:"#9a8f7e", textTransform:"uppercase", fontWeight:700, marginBottom:12}}>Valores por Dificultad</p>
                  {["Fácil", "Intermedio", "Difícil"].map(diff => (
                    <div key={diff} style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8}}>
                      ...
                      <div style={{display:"flex", alignItems:"center", gap:8}}>
                        ...
                        <span style={{fontSize:13, fontWeight:700, minWidth:50, textAlign:"center"}}>{fmt(rates[diff])}</span>
                ...
                {/* --- MULTIMEDIA --- */}
                <div style={{background:"#fff", border:"1px solid #E8E5DC", borderRadius:16, padding:16}}>
                  <p style={{fontSize:10, color:"#9a8f7e", textTransform:"uppercase", fontWeight:700, marginBottom:12}}>Multimedia</p>
                  {[
                    {label:"✓ Imagen Acierto", type:"success"},
                    {label:"✗ Imagen Error", type:"error"},
                    {label:"Icono App", type:"icon"}
                  ].map(item => (
                    <label key={item.type} style={{display:"flex", alignItems:"center", gap:10, background:"#FAFAF7", padding:10, borderRadius:10, marginBottom:6, cursor:"pointer"}}>
                      <Upload size={14} color="#9a8f7e"/>
                ...
                <button onClick={() => { setSettingsOpen(false); goHome(); }} style={S.btnPrimary}>Cerrar Configuración</button>
              </div>
            )}
...
const S = {
  ...
  btnPrimary: { width: "100%", maxWidth: 320, background: "#1a1a2e", color: "#F5F0E8", fontWeight: 700, padding: 16, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 10 },
  ...
  btnSmall: { background: "#1a1a2e", color: "#fff", width: 28, height: 28, borderRadius: 6, fontWeight: 800 },
